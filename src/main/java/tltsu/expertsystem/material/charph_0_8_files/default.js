function contentOperation(form)
{
    ...
    this.doSend = function()
    {
        var text = null;
	if(!this.is_auth) {
		alert("Эта функция доступна только авторизованным пользователям. Авторизуйтесь и потом сообщите об ошибке, пожалуйста.");
		return;
	}
        if (navigator.appName.indexOf("Netscape")!=-1 && eval(navigator.appVersion.substring(0,1))<5) {
            alert("Браузер не поддерживает данную функцию.");
            return;
        }
        var selection = null;
